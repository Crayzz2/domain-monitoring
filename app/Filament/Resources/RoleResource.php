            ...
            ->columns([
                ...
                TextColumn::make('name')
                    ...
                    ->formatStateUsing(function($state){
                        $role = Role::where('name', $state)->get();
                        $roleHasPermission = DB::table('role_has_permissions')->where('role_id', $role->pluck('id')->first())->get();
                        if(!$roleHasPermission->first()){
                            return $state;
                        }
                        $finalText = $state . ' - ';
                        foreach($roleHasPermission as $permissionId){
                            $permission = Permission::find($permissionId->permission_id);
                            $finalText .= ' ' . $permission->name  . ' |';
                        }
                        return substr_replace($finalText, ' ', -1);
                    })
                    ->wrap()
                    ->searchable(),
